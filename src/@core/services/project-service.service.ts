...
export class ProjectServiceService {
  ...
  constructor(private http: HttpClient,private router:Router) {
    this.BackEndURL = 'http://localhost:8089/work-mood/Projects/';
    }
   ...
   public save(event: Project) {
     console.log(Project);
   return this.http.post(this.BackEndURL+"AddProject",event,{ ...options, responseType: 'text' });
   }
   /*Supprimer Project*/
   DeleteProject(id:number){
       return this.http.delete(this.BackEndURL+'DeleteProject/'+id).pipe(
         map(
           projectData => {
   ...
   UpdateProject(event:Project){
        return this.http.put(this.BackEndURL+'UpdateProject',event).pipe(
           map(
             projectData => {
             }
           )
